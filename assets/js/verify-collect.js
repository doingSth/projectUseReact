...
var verify = new function() {
    ...
    function getCss(elm) {
        return {left: $(elm).offset().left - verifyLeft,top: $(elm).offset().top - verifyTop,width: $(elm).width(),height: $(elm).height()}
    }
	...
	verifyLeft = $verify.offset()&&$verify.offset().left, verifyTop = $verify.offset()&&$verify.offset().top, spritesLeft = $verifyImgSprites.offset().left,
	spritesTop = $verifyImgSprites.offset().top, spritesWidth = $verifyImgSprites.width(), 
	...
	this.initPos = function(){
		...
		//登录提交的参数
	        var ajaxData = {'channelIdList':channelId,'needValidateCodeList':channelDicId+"_"+channelId,'isNew':0,'isTalent':0};
	        ajaxData[channelDicId+"_"+channelId] = JSON.stringify(collectData);//该参数代替之前的验证码即可
	        ajaxData[channelDicId] = channelId;
